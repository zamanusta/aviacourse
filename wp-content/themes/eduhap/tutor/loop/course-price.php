...
if ( ! is_user_logged_in() ) {
	...
	$enroll_btn = '<div class="tutor-course-list-btn">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block ' . $button_class . ' " data-course-id="' . $course_id . '">' . __( 'Enroll Course', 'eduhap' ) . '</a>' ) . '</div>';
...
if ( tutor_utils()->is_course_purchasable() ) {
	...
	if ( 0 != $maximum_students && $total_enrolled != $maximum_students ) {
		...
		echo '<div class="tutor-d-flex tutor-align-center tutor-justify-between">
                    ...
                    <div class="tutor-course-booking-progress tutor-d-flex tutor-align-center">
						...
						esc_html( $b_total ) . __( '% Booked', 'eduhap' ) . '
                        </div>
                    </div>
                </div>
	...
	if ( $product && $maximum_students == $total_enrolled && 0 != $maximum_students ) {
		...
		$restrict   = '<div class="list-item-booking booking-full tutor-d-flex tutor-align-center"><div class="booking-progress tutor-d-flex"><span class="tutor-mr-8 tutor-color-warning tutor-icon-circle-info"></span></div><div class="tutor-fs-7 tutor-fw-medium tutor-color-black">' . __( 'Fully Booked', 'eduhap' ) . '</div></div></div>';
